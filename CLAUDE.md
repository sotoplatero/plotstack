...
dashboard/app.js  --chrome.runtime.sendMessage-->  src/background.js
                                                        |
                              src/providers/substack-api.js      (snapshot central)
                              src/providers/substack-extended.js (fuentes opcionales)
                                                        |
...
- **`src/background.js`** — único punto que toca la red y el almacenamiento. Gestiona conexión, sincronización y desconexión. Persiste tres claves: `plotstack.snapshot`, `plotstack.connection`, `plotstack.analytics`.
- **`src/providers/substack-api.js`** — `getPublicationSnapshot()` es la función más pesada del proyecto: dispara summary/summary-v2/email_stats/posts/notas en `Promise.allSettled`, pagina `post_management/published` y el feed de perfil, y luego resuelve detalles por post y por nota con `mapWithConcurrency(…, 5, …)`.
- **`src/providers/substack-extended.js`** — **nueve** fuentes opcionales: `audience`, `subscriberTimeline`, `growthSources`, `followerTimeseries`, `audienceLocation`, `freeSubscriberGrowth`, `paidSubscriberGrowth`, `freeRetention`, `paidRetention`. Se retiraron seis (red 500, recomendaciones 400, payment_pledges 400, planes, inventario y exportaciones) porque no alimentaban ningún renderer. **Una fuente sin consumidor no se sincroniza.** Cada una tiene `request` + `normalize`; se ejecutan con `Promise.allSettled` y producen un array `coverage` que el dashboard muestra en el panel **Cobertura**.
...
`cobertura`. **Sin vista de ingresos: se retiró por decisión de producto, junto a
...
Resumen es deliberadamente corto: suscriptores, apertura, CTR y crecimiento.
...
- **Sincronización incremental.** Posts y notas reutilizan el snapshot previo: solo se refrescan los 12 más recientes más los que aún no tienen detalle (`detailAvailable` / `stats.available`). No conviertas esto en un refresco total.
...
`tests/background.test.js` stubea `chrome.*` antes de importar el service worker y
captura el listener de mensajes para ejercitar los flujos reales.
