...
- **`src/shared/content-analytics.js`** — módulo puro, sin red y sin persistencia. **`getContentFindings()` y la tabla de rasgos se retiraron de la interfaz** (el análisis por medianas no era útil con pocas notas; se sustituirá por IA). Siguen exportadas y con tests, pero **ningún renderer las consume**. Lo que el dashboard sí usa, a través de la fachada `getContentAnalytics()`: `getCadenceCalendar` (calendario estilo GitHub: una celda por día, columnas por semana alineadas al lunes, tope de 53 semanas con `truncatedWeeks` declarado; es lo que se pinta), `getCadenceHeatmap` (sigue exportada: sus `buckets` por tramo horario alimentan solo el badge "día · tramo más activo"; las medianas de tramo salen de los valores crudos, nunca de re-derivar medianas) y `getNoteAttributionTimeline` (panel "Altas atribuidas a notas" de la vista Notas).
